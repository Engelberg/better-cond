...
(defnc f [x {:keys [k1 k2] k3 :k3 :as m} [v1 v2 :as v]]
  :let [k4 (:k4 m)]
  [x k1 k2 k3 k4 v])

(defnc- g [x {:keys [k1 k2] k3 :k3 :as m} [v1 v2 :as v]]
  :let [k4 (:k4 m)]
  [x k1 k2 k3 k4 v])
...
(deftest better-cond
  (are [x y] (= x y)
    ...
    nil (when-some [x nil] 2)
    2 (when-some [x false] 2)
    [true true] (when-some [x true, y true] [x y])
    nil (when-some [x true, y nil] [x y])
    true (when-let [x true] x)
    nil (when-let [x nil] 2)
    nil (when-let [x false] 2)    
    [true true] (when-let [x true, y true] [x y])
    nil (when-let [x true, y nil] [x y])
    true (if-let [x true y true] x 3)
    3 (if-let [x false [y] [1]] x 3)
    false (if-some [x false {y :y} {:y 1}] x 3)))
